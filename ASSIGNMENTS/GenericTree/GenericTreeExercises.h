...
static void treeFactory(GenericTree<int>& tree) {
  ...
  // TreeNode* root = getRootPtr(tree);

  // Clearing the tree passed in. 
    tree.clear();

  // Making a root treenode and adding it to the top of the tree. 
    tree.createRoot(4);

  // Getting the root pointer of the tree and adding children;
...
std::vector<T> traverseLevels(GenericTree<T>& tree) {
  ...
  // Begin by pushing our root pointer onto the stack
  nodesToExplore.push(rootNodePtr);

  // Push the data of the nodes in the stack to results, then traverse down
  while (!nodesToExplore.empty()) {
