...
void LinkedList<T>::insertOrdered(const T& newData) {
  if (tail_ != nullptr) {
    // std::cout << "current tail = "<< tail_->data << std::endl;
  }
  ...
  // std::cout<< "size = (first two clauses)" << size_ << std::endl;
    ...
    if (newNode->data < head_->data) {
      ...
      while (cur->next != nullptr) {
        ...
        if (cur->data > newNode->data) {
          ...
          newNode->prev = prev;
          newNode->next = cur; 
          // std::cout << "inserted middle"<< std::endl;
          break;
        }
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T>& other) const {
  ...
  if (!left.empty()) {
    ...
    // Node* leftTail = left.tail_;
    // Node* mergedTail = merged.tail_;
  
    // merged.tail_ = leftTail;
    // merged.size_ += left.size();
    // leftHead->prev = mergedTail;
    ...
    // Node* rightTail = right.tail_;
    // Node* mergedTail = merged.tail_;
    
    // merged.tail_ = rightTail;
    // merged.size_ += right.size();
    // rightHead->prev = mergedTail;
  ...
  // std::cout << "Head prev is nullptr: " << merged.head_->prev << std::endl;
  // std::cout << "Tail next is nullptr: " << merged.tail_->next << std::endl;
  // std::cout << "size after merged: " << merged.size()<< std::endl;
